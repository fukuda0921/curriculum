...
-- [SELECT * FROM stocks_table JOIN store_table ON stocks_table.store_code = store_table.store_code;]
--[SELECT * FROM stocks_table JOIN goods_table ON stocks_table.goods_code = goods_table.goods_code;]
...
-- [SELECT store_code, quantity FROM stocks_table WHERE quantity > (SELECT AVG (quantity) FROM stocks_table WHERE store_code = 'EA01')]
 
 
-- 6. 商品テーブルに「商品コード='M001'、商品名='マフラー'、単価=4500円、更新日付=本日日付」のデータを追加しなさい。※実行後のSELECT結果も貼付すること。
-- [INSERT INTO goods_table (goods_code , goods_name , price , update_day) VALUES ('M001','マフラー','4500','now');]
